...
| `assets/source/media/client-latest-2026-08-02/processed-transparent/Rice.png`, `Stake_nePapa.png`, `Ulusu_nedombolo.png`, `Checkin.png`, `Starter.png`, `Mautwana.png`, `Noddles.png` (7 files) | Client (background-removed dish cutouts) | PNG, 1440×1440, RGBA, verified true alpha (IDAT decoded, not assumed from colour type) | Usable — pending batch confirmation | — | Clean cutouts, no halo, natural colour, plates/bowls/containers preserved. Visually checked against all 9 Rejected/Flagged source images above; none match. Provenance/usage-rights for the whole 2026-08-02 batch not yet confirmed by Tebogo — see Client Inputs Register I-018. |
| `assets/source/media/client-latest-2026-08-02/processed-transparent/Ulusu.png` | Client (background-removed) | PNG, 1440×1440, RGBA | Usable — needs retouch | — | Same batch as above; a floating utensil fragment survives top-right of the cutout. Do not ship until retouched. |
| `assets/source/media/client-latest-2026-08-02/processed-transparent/Green-plate.png` | **Not Lina — do not use** | PNG, 1440×1440, RGBA | **Rejected** | — | Background-removed version of `instagram_1785244457096.png` (above), confirmed by direct visual comparison — identical bowl, bone angle, microgreen placement, sauce pooling. That source is already Rejected ("Lamb shank, mash, microgreens on stoneware. Fine-dining plating.") with unconfirmed usage rights under I-012. This is the single most premium-looking image in the 2026-08-02 batch and is the one most likely to be reached for by mistake — flagged here explicitly so it is never used. |
| `lina-dish-steak-pap-working-v1-20260802.webp` | Derived from `Stake_nePapa.png` (ffmpeg, scale=800:-1, libwebp q82) | WebP, 800×800, RGBA (yuva420p, alpha confirmed) | **Live — deployed and in use** (allowlisted in `.vercelignore`) | — | ~68KB. Original untouched. Built for the D-025 numbered-index homepage direction; that direction is live in the current build (D-031) — currently the homepage's "Explore Our Menu" destination image. Usage rights fall under the open I-018 batch confirmation, same as every other 2026-08-02 derivative below. |
| `lina-dish-tripe-dumpling-working-v1-20260802.webp` | Derived from `Ulusu_nedombolo.png` | WebP, 800×800, RGBA | **Live — deployed and in use** (allowlisted in `.vercelignore`) | — | ~47KB. Original untouched. Currently the homepage's "Order Today" destination image (D-031). Subject to I-018. |
| `lina-dish-catering-bowl-working-v1-20260802.webp` | Derived from `Checkin.png` | WebP, 800×800, RGBA | **Live — deployed and in use** (allowlisted in `.vercelignore`) | — | ~63KB. Original untouched. Currently used on the Catering page hero and the About Lina's page. Subject to I-018. |
| `lina-dish-rice-working-v1-20260802.webp` | Derived from `Rice.png` | WebP, 800×800, RGBA | **Live — deployed and in use** (allowlisted in `.vercelignore`) | — | Best single cutout in the batch. No longer held in reserve: currently used on the homepage's "Our Offerings" section ("Today's Menu"), the Catering page, and the About Lina's page. Subject to I-018. |
...
## Poster-led homepage first-screen build (2026-08-02)
New working files under `assets/mockups/working/prototype-v2/`: `home-media.js` (central media/copy configuration — every image path, focal point, alt text and destination link for the first screen lives here, nothing hardcoded in `index.html` or `home.js`), `home.css` (scoped under `body.home`/`.home-*`, loaded after `styles.css`), `home.js` (builds the three destination rows and wires the desktop hover-reveal / mobile two-tap interaction). `index.html`'s first screen replaced; header, drawer, footer and all existing script wiring preserved unchanged. Full rationale, slot assignments and risk notes: Decision Log D-025.

**Four WebP derivatives produced this round** (ffmpeg already installed/authorised in a prior session — not reinstalled), from the 2026-08-02 `processed-transparent` cutout batch:
| File | Source | Command | Purpose |
|---|---|---|---|
| `lina-dish-steak-pap-working-v1-20260802.webp` | `Stake_nePapa.png` | `ffmpeg -i <src> -vf scale=800:-1 -c:v libwebp -quality 82 -compression_level 6 <out>` | Homepage "Explore Our Menu" destination |
| `lina-dish-tripe-dumpling-working-v1-20260802.webp` | `Ulusu_nedombolo.png` | same | Homepage "Order Today" destination |
| `lina-dish-catering-bowl-working-v1-20260802.webp` | `Checkin.png` | same | Homepage "Plan an Event" destination |
| `lina-dish-rice-working-v1-20260802.webp` | `Rice.png` | same | Produced, held in reserve — not deployed this round |

All four confirmed 800×800, `pix_fmt=yuva420p` (genuine alpha preserved, checked via `ffprobe`), 47–71KB each — well under the ≤120KB target. Source PNGs (22MB total) remain untouched under `assets/source/`, which `.vercelignore` never deploys.

**Verification:** `admin:check` clean, 90/90 unit tests, 51/51 rules tests, 0 vulnerabilities (all unchanged from pre-task baseline). Headless Chromium (`playwright-core`, installed `--no-save` for this session only, driven against the cached browser binary already present on this machine — not persisted to `package.json`) at 1440/1280/768/390px: zero horizontal overflow, zero console errors, first screen fits exactly within `100svh`/`100dvh` at every width with no scroll. Keyboard Tab reaches logo → MENU → all 3 destinations → scroll cue, each with a visible focus ring; MENU opens the existing drawer via Enter, Escape closes it and returns focus, confirming `script.js`'s drawer logic is untouched. Touch emulation (`hasTouch`/`isMobile`) confirmed the two-tap contract precisely: first tap expands a row in place with no navigation, a second tap on the same row navigates. Desktop hover confirmed the dish-cutout crossfade and active-row red rule. Screenshots: `assets/mockups/working/client-review-poster-led/*.png`.

## Poster-led homepage rebuild — split editorial hero (2026-08-02, same day)
The numbered-index direction above (destinations, crossfade well, mobile accordion) is **superseded**, not extended — see Decision Log D-026. `home-media.js`/`home.css`/`home.js` rewritten in place; `index.html`'s header and first screen replaced again. The four WebP derivatives listed above are no longer referenced by any page and their `.vercelignore` lines were removed; the files stay on disk, held in reserve.

**Hero image:** `lina-hero-poster-working-v1-20260728.jpg` (existing derivative, unchanged) — the real confirmed chef photo, used directly as a bordered photographic panel. No background-removed chef cutout exists anywhere in the project (confirmed by a fresh search of `assets/source/media/` and `assets/mockups/working/media/`); the only other chef-identifying image remains excluded per D-023. No cutout was fabricated — this environment has no background-removal capability, the same limitation already surfaced and accepted earlier in this session for an unrelated request.

**Verification (rebuild):** `admin:check` clean, 90/90 unit tests, 51/51 rules tests, 0 vulnerabilities — re-confirmed unchanged after the rebuild. Zero horizontal overflow, zero console errors, first screen fits exactly within `100svh`/`100dvh` at 1440/1280/768/390px. Keyboard Tab confirmed walking the new inline nav (Home/Catering/Menu/Meet Chef Lina/Find Us) → header CTA → MENU → hero CTA with a visible focus ring throughout; drawer open/Escape/focus-return re-confirmed working under the new header markup. `prefers-reduced-motion: reduce` confirmed collapsing the new fade-in to immediate full opacity. One bug found and fixed: the ribbon's separator dot could orphan onto its own line on mobile — moved to a CSS `::before` on each label so it can't be split by wrapping. Screenshots: `assets/mockups/working/client-review-poster-led/v2-*.png`.

## Chef Lina background removed — hero cutout (2026-08-02, same day)
`assets/source/media/client-latest-2026-08-02/processed-transparent/Lina's_Welcome.png` | Client-supplied, background-removed | PNG, 1440×1920, RGBA, verified true alpha (IHDR/IDAT decoded — alpha=0 at background samples, alpha=255 over chef/counter) | Usable — see I-018 for the same batch-provenance item covering this file | — | Trailer's upper interior removed; counter, plate and red "Lina's / CHEF LINA" signage remain attached as one unit, not a fully isolated silhouette — reads as intentional grounding, not a defect. See Decision Log D-027.

`lina-hero-chef-cutout-working-v1-20260802.webp` | Derived from `Lina's_Welcome.png` (ffmpeg, scale=900:-1, libwebp q90) | WebP, 900×1200, RGBA (`yuva420p`, alpha confirmed) | **Live — deployed and in use** (allowlisted in `.vercelignore`), reused for a different purpose than originally built for | — | ~73KB. Original untouched. Was the homepage *hero* visual under D-027, then replaced there by `lina-welcome-hero-working-v1-20260802.webp` under D-028 (that supersession, for the hero slot specifically, still stands). Later reused — separately from the hero — as the Catering/About Lina's page hero image and the homepage "Our Offerings" Catering-card image (D-031). Not superseded for those uses; correcting this row's earlier "no longer referenced" status, which predates that reuse. Subject to I-018.

## Chef Lina hero cutout replaced with a fully isolated version (2026-08-02, same day)
`assets/source/media/client-latest-2026-08-02/processed-transparent/Lina's Welcome.png` | Client-supplied, background-removed | PNG, 1800×2400, RGBA, verified true alpha (IHDR/IDAT decoded — alpha=0 at all 4 corners plus interior background samples) | Usable — see I-018 for the same batch-provenance item covering this file | — | A different, more complete export than D-027's file: trailer counter and signage also removed, a genuine full-body isolated silhouette holding the plate. See Decision Log D-028.

`lina-welcome-hero-working-v1-20260802.webp` | Derived from `Lina's Welcome.png` (ffmpeg, scale=1000:-1, libwebp q92) | WebP, 1000×1333, RGBA (`yuva420p`, alpha confirmed) | Used — current homepage hero visual | — | ~102KB. Original untouched.

## Interactive digital menu v1.0 REVIEW (2026-08-03)

| Asset | Source/owner | Format | Status | Approved by | Notes |
|---|---|---|---|---|---|
| `assets/menu/working/digital-menu-v1.0/LINAS_Digital-Menu_Interactive_v1.0_REVIEW_2026-08-03.pdf` | Generated from the authoritative menu PDF, existing `menu-data.js`, confirmed brand assets and confirmed contact details | PDF, A4 portrait, 8 pages, 1.47 MiB | **Produced - REVIEW** | Change Contract approved by Tebogo; final exported asset not yet client-approved | 61 interactive links; selectable text; embedded font subsets; no cart deep-link claim; production customer routes remain behind Coming Soon |
| `assets/menu/working/digital-menu-v1.0/LINAS_Digital-Menu_Source_v1.0_REVIEW_2026-08-03.html` | Project-produced editable source | HTML | **Produced - REVIEW source** | Tebogo approved creation | Reads the existing website menu data; layout and content structure remain editable |
| `assets/menu/working/digital-menu-v1.0/LINAS_Digital-Menu_Styles_v1.0_REVIEW_2026-08-03.css` | Current poster-led website direction + verified brand foundation | CSS | **Produced - REVIEW source** | Tebogo approved visual reference | A4 print layout using Lina Red, warm white, charcoal/wine, Georgia and Segoe UI |
| `assets/menu/working/digital-menu-v1.0/LINAS_Digital-Menu_Config_v1.0_REVIEW_2026-08-03.js` | Tebogo-confirmed business details, 3 Aug 2026 | JavaScript config | **Produced - REVIEW source** | Tebogo | Central website, menu, catering, WhatsApp, email, Instagram, address, hours and directions configuration |
| `assets/menu/working/digital-menu-v1.0/LINAS_Digital-Menu_Link-Manifest_v1.0_REVIEW_2026-08-03.csv` | Generated QA evidence | CSV | **Verified** | - | One row per visible PDF action; 61/61 PASS |
| `assets/menu/working/digital-menu-v1.0/LINAS_Digital-Menu_QA_v1.0_REVIEW_2026-08-03.md` | Generated + inspected QA evidence | Markdown | **Verified** | - | Content reconciliation, link, PDF, accessibility, visual and regression results |

**Images used:** only the confirmed Lina's logo, the confirmed chef/trailer photograph, a confirmed mobile-kitchen preparation frame and a confirmed event-preparation frame. No pending-provenance 2026-08-02 dish cutout and no rejected media was used.
